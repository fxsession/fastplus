 ...
 * @author Dmitry Vulf
 * 
 *         Contains methods and structures to implement classical orderbook Can
 *         be applied for different objects
 */
public class FPFOrderBook implements IFPFTransaction {
	...
	/*
	 * private maps containing sorted by price orders books for bid and ask key
	 * - price, value - size
	 */
	private final TreeMap<Double, Integer> bidBook = new TreeMap<Double, Integer>(
			Collections.reverseOrder());

	private final TreeMap<Double, Integer> askBook = new TreeMap<Double, Integer>(
			Collections.reverseOrder());
	...
	public FPFCommand dispatch(String action, String type, String size,
			String price) throws FXPException {
		FPFCommand retval = FPFCommand.ON_NULL;
		...
		if (type.equals(T_BID)) {
			...
			case T_DELETE:
				...
				break;
			default:
				throw new FXPException("unknown operation");
			}
			retval = FPFCommand.ON_CHANGED_BID;
			...
			retval = FPFCommand.ON_CHANGED_ASK;
		}
		return retval;
	}

	/*
	 * bid book keeps key in the reverse order. highest rate(1st entry) - best
	 * deal
	 * 
	 * ask book - keeps direct order. lowest rate(1st entry) - best deal.
	 * 
	...
	public void addBid(String size, String px) throws FXPException {
		...
		if (mylogger.isDebugEnabled())
			mylogger.debug(instrumentId + " bid   a  "
					+ FXPUtils.Double2String(dpx, 5) + " " + isize);
	}
	...
	public void deleteBid(String size, String px) throws FXPException {
		...
		bidBook.remove(dpx);
		if (mylogger.isDebugEnabled())
			mylogger.debug(instrumentId + " bid   d  "
					+ FXPUtils.Double2String(dpx, 5) + " " + isize);
	}

	private String scanBid() {
		/*
		 * a template how to visualize bid orderbook best bid(maximum) - on the
		 * top
		 */

		String retval = "";
		for (Map.Entry<Double, Integer> entry : bidBook.entrySet()) {
			retval += FXPUtils.Double2String(entry.getKey(), 5) + " "
					+ entry.getValue() + "\r\n";
		}
	 ...
	 * 
	 * topLevels2skip - skip N levels in calculations. Them most liquid levels
	 * usually are closed "immediately" so there is no much sense to count them
	 */
	...
	public void addAsk(String size, String px) throws FXPException {
		...
		if (mylogger.isDebugEnabled())
			mylogger.debug(instrumentId + " ask   a                      "
					+ FXPUtils.Double2String(dpx, 5) + " " + isize);
	}
	...
	public void deleteAsk(String size, String px) throws FXPException {
		...
		askBook.remove(dpx);
		if (mylogger.isDebugEnabled())
			mylogger.debug(instrumentId + " ask   d                      "
					+ FXPUtils.Double2String(dpx, 5) + " " + isize);
	}

	public String scanAsk() {
		/*
		 * a template how to visualize bid orderbook best ask(minimum) - on the
		 * bottom
		 */
		String retval = "";
		for (Map.Entry<Double, Integer> entry : askBook.entrySet()) {
			retval += "                       "
					+ FXPUtils.Double2String(entry.getKey(), 5) + " "
					+ entry.getValue() + "\r\n";
		}
		return retval;
	}
	...
	public Double getAskVWAP(Integer size, Integer topLevels2skip) {
		...
		Integer intrsize = size;
		Integer level = 0;
		for (Map.Entry<Double, Integer> entry : askBook.descendingMap()
				.entrySet()) {
			Integer newsize = entry.getValue();
			if (level >= topLevels2skip) {
				if (newsize < intrsize) { // take full size of the level

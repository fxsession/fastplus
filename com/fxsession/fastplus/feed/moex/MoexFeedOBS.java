...
/**
 * @author Dmitry Vulf Order Book snapshot
 * 
 */
public class MoexFeedOBS extends MoexFeed {
	private static Logger mylogger = Logger.getLogger(MoexFeedOBS.class);
	...
	public void processMessage(Message message) throws FastConnectionException {
		...
		try {
			if (message.getTemplate().getId().equals(getTemplateID())) {
				...
				value = message.getString(FPFMessage
						.getFieldName(LASTMSGSEQNUMPROCESSED));
